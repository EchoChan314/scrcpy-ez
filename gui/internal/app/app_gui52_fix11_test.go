...
func TestGui52Fix11MergeKeepsMainActiveTls(t *testing.T) {
  ...
  "devices": {
    "192.0.2.162:33301": {
      "addrs": [{"addr": "192.0.2.162:33301", "state": "active", "mode": "tls"}],
      "profiles": {"usb": {}, "wifi": {}}
    },
    "Xiaomi Pad 8 Pro": {
      ...
      "addrs": [
        {"addr": "192.0.2.162:5555", "state": "active", "mode": "tcpip"},
        {"addr": "192.0.2.162:42145", "state": "active", "mode": "tls", "lastOk": 1788354318}
      ],
      "profiles": {"usb": {}, "wifi": {}}
    }
  },
  "deviceOrder": ["192.0.2.162:33301", "Xiaomi Pad 8 Pro"]
}`
	...
	if !gui50Fix45EntryHasAddr(e, "192.0.2.162:42145", ModeTls) {
		t.Fatalf("mdns 权威 42145 应保留 active: %+v", e.Addrs)
	}
	for i := range e.Addrs {
		if e.Addrs[i].Addr == "192.0.2.162:33301" && e.Addrs[i].State == AddrStateActive {
			t.Fatalf("配对瞬时 transport 33301 不得为主 active: %+v", e.Addrs)
		}
	}
	// 孤儿键应被删除（合并后不进 deviceOrder）
	keys := s.Entries()
	if _, exists := keys["192.0.2.162:33301"]; exists {
...
func TestGui52Fix11MergeAppendsWhenNoMainActive(t *testing.T) {
  ...
  "devices": {
    ...
    "Xiaomi Pad 8 Pro": {
      ...
      "addrs": [
        {"addr": "192.0.2.162:5555", "state": "active", "mode": "tcpip"},
        {"addr": "192.0.2.162:41999", "state": "stale", "mode": "tls"}
      ],
      "profiles": {"usb": {}, "wifi": {}}
    }
  },
  "deviceOrder": ["192.0.2.162:33301", "Xiaomi Pad 8 Pro"]
	...
	for i := range e.Addrs {
		if e.Addrs[i].Addr == "192.0.2.162:33301" {
			t.Fatalf("零合入：孤儿地址不得出现在主档案: %+v", e.Addrs)
		}
	}
	if !gui50Fix45EntryHasAddr(e, "192.0.2.162:5555", ModeTcpip) {
