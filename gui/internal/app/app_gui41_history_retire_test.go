...
func TestGui41HistoryRetireDeletes(t *testing.T) {
	...
	gui15Seed(s, "REDMI K80", &DeviceEntry{
		...
		Addrs: []AddrEntry{
			...
			{Addr: "192.0.2.197:40000", State: AddrStateActive, LastOk: 200, Mode: ModeTls},
			{Addr: "192.0.2.197:5555", State: AddrStateActive, LastOk: 50, Mode: ModeTcpip},
		},
	...
	if gui24FindAddr(e, "192.0.2.197:33895") != nil {
		t.Fatalf("旧同形态条目应被删除（history 退役）: %+v", e.Addrs)
	}
	if gui24FindAddr(e, "192.0.2.197:40000") == nil {
		t.Fatalf("新同形态条目应保留: %+v", e.Addrs)
	}
	if gui24FindAddr(e, "192.0.2.197:5555") == nil {
...
func TestGui41NormalizeClearsHistory(t *testing.T) {
	...
	gui15Seed(s, "REDMI K80", &DeviceEntry{
		...
		Addrs: []AddrEntry{
			// history 旧端口
			{Addr: "192.0.2.197:33895", State: AddrStateHistory, LastOk: 150, Mode: ModeTls},
			// 同形态两条 active：只应保留 lastOk 最新 45000
			{Addr: "192.0.2.197:40000", State: AddrStateActive, LastOk: 100, Mode: ModeTls},
			{Addr: "192.0.2.197:45000", State: AddrStateActive, LastOk: 200, Mode: ModeTls},
			{Addr: "192.0.2.197:5555", State: AddrStateActive, LastOk: 50, Mode: ModeTcpip},
		},
		Profiles: DefaultProfile(),
	})
	...
	tls := gui24FindAddr(e, "192.0.2.197:45000")
	if tls == nil {
...
func TestGui41NewestOfClassNoHistoryFallback(t *testing.T) {
	...
	if !ok || a.Addr != "192.0.2.197:33895" || a.State != AddrStateStale {
