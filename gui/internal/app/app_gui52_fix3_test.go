...
func TestGui52Fix3RuntimeCleanMergesOrphan(t *testing.T) {
	...
	s.mu.Lock()
	s.data.Devices["192.0.2.162:42595"] = &DeviceEntry{
		Addrs:    []AddrEntry{{Addr: "192.0.2.162:42595", State: AddrStateActive, Mode: ModeTls}},
	...
	s.data.DeviceOrder = []string{"192.0.2.162:42595", "Xiaomi Pad 8 Pro"}
	s.mu.Unlock()
	...
	if !gui50Fix45EntryHasAddr(main, "192.0.2.162:5555", ModeTcpip) {
		t.Fatalf("主档案权威地址应保留: %+v", main.Addrs)
	}
	for i := range main.Addrs {
		if main.Addrs[i].Addr == "192.0.2.162:42595" {
	...
	mdns8LogContains(t, logPath, "孤儿档案清除（零合入）：192.0.2.162:42595 → Xiaomi Pad 8 Pro（事件驱动）")
...
func TestGui52Fix3RuntimeCleanKeepsOrphanWithoutMain(t *testing.T) {
	s := NewProfileStore("")
	s.mu.Lock()
	s.data.Devices["192.0.2.184:38167"] = &DeviceEntry{
		Addrs:    []AddrEntry{{Addr: "192.0.2.184:38167", State: AddrStateActive, Mode: ModeTls}},
		Profiles: DefaultProfile(),
	}
	s.data.Devices["Xiaomi Pad 8 Pro"] = &DeviceEntry{
		Marketname: "Xiaomi Pad 8 Pro",
...
func TestGui52Fix3PairArchiveTriggersMerge(t *testing.T) {
	...
	s.data.Devices["Xiaomi Pad 8 Pro"] = &DeviceEntry{
		...
		Addrs: []AddrEntry{
			{Addr: "192.0.2.162:5555", State: AddrStateActive, Mode: ModeTcpip},
			{Addr: "192.0.2.162:44125", State: AddrStateActive, Mode: ModeTls},
		},
		Profiles: DefaultProfile(),
	}
	s.mu.Unlock()
	...
	if !gui50Fix45EntryHasAddr(main, "192.0.2.162:44125", ModeTls) {
		t.Fatalf("主档案同形态 active 应保留: %+v", main.Addrs)
	}
	for i := range main.Addrs {
		if main.Addrs[i].Addr == "192.0.2.162:42595" && main.Addrs[i].State == AddrStateActive {
			t.Fatalf("过渡地址不得为主 active: %+v", main.Addrs)
		}
	}
	raw := readFileString(t, filepath.Join(dir, "profiles.json"))
	if strings.Contains(raw, `"192.0.2.162:42595":`) {
...
func TestGui52Fix3TrackUpdateTriggerMerges(t *testing.T) {
	...
	d := adb.Device{Serial: "192.0.2.162:42595", State: "device", ConnType: "wifi"}
...
func TestGui52Fix3UnifyDisplayDefense(t *testing.T) {
	a, _ := newWirelessApp()
	a.profiles.mu.Lock()
	a.profiles.data.Devices["192.0.2.162:42595"] = &DeviceEntry{
		Addrs:    []AddrEntry{{Addr: "192.0.2.162:42595", State: AddrStateActive, Mode: ModeTls}},
		Profiles: DefaultProfile(),
	}
	...
	out := a.unifyProfileCards([]adb.Device{
		{Serial: "192.0.2.162:42595", State: "device", ConnType: "wifi"},
	})
	if len(out) != 1 {
		t.Fatalf("过渡档不得独立成卡: %+v", out)
	}
	if _, ok := a.profiles.Entries()["192.0.2.162:42595"]; ok {
		t.Fatal("unify 前应完成事件合卡（孤儿键删除）")
	}
	// gui52-fix12：孤儿零合入——过渡地址不再进入主档案，显示用主档案 active（5555）。
	if out[0].Serial != "192.0.2.162:5555" || out[0].Tls {
