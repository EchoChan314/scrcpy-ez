...
func TestStartCastInjectsSerialForUSB(t *testing.T) {
	...
	a2.mu.Lock()
	a2.devices = []adb.Device{{Serial: "TEST0002", State: "device", ConnType: "usb"}}
	a2.mu.Unlock()
	_ = a2.StartCast("TEST0002")
	if p2 := f2.waitParams(t, 1); p2.Serial != "TEST0002" || p2.Addr != "" || p2.NoWatch {
...
func TestStartCastNoWatchOnlyForParallel(t *testing.T) {
	...
	a2.mu.Unlock()
	_ = a2.StartCast("192.0.2.162:5555")
...
func TestStartCastInjectsAddrForWifi(t *testing.T) {
	...
	a2.SetRunnerFactory(func(string, func(string), func(int)) (Runner, error) { return f2, nil })
	a2.mu.Lock()
	a2.devices = []adb.Device{{Serial: "192.0.2.99:5555", State: "device", ConnType: "wifi"}}
	a2.mu.Unlock()
	_ = a2.StartCast("192.0.2.99:5555")
	if p2 := f2.waitParams(t, 1); p2.Addr != "192.0.2.99:5555" {
...
func TestAppStartLegacyMigration(t *testing.T) {
	...
	legacy := `{"TEST0002": {"usb": {"res": 2400, "fps": 75, "bitrate": 55, "custom": true,
...
func TestModeFollowsRealModeLines(t *testing.T) {
	...
	for _, l := range []string{
		"[1] 重置 adb 服务...",
		"[2] 检测 USB 设备...",
		"[OK] 检测到 USB 设备：Xiaomi Pad 8 Pro（TEST0002）",
		"[学习] 有线模式：检测手机 WiFi IP...",
		"[OK] 检测到手机 WiFi IP：192.0.2.197",
		"[OK] 已保存无线地址到 config.txt：192.0.2.162:5555",
	} {
		a.NotifyLine("X", l)
	}
	...
	a.NotifyLine("X", "[提示] 已保存无线地址到 config.txt：192.0.2.162:5555")
	if s := a.Snapshot().Cast; s.Mode != "usb" {
		t.Fatalf("保存无线地址行不得改变模式: %q", s.Mode)
	}
	// 规格定案
	a.NotifyLine("X", "===== 开始投屏：Xiaomi Pad 8 Pro（TEST0002） =====")
	...
	for _, l := range []string{
		...
		"[3] 无线模式：尝试连接上次保存的地址...",
		"[OK] 连接成功：Xiaomi Pad 8 Pro（192.0.2.162:5555）",
		"[OK] 已保存无线地址到 config.txt：192.0.2.162:5555",
		"===== 开始投屏：Xiaomi Pad 8 Pro（192.0.2.162:5555） =====",
